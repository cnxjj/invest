# https://zhuanlan.zhihu.com/p/350578719
# 获取股票K线数据
...
if __name__ == "__main__":
    # 股票代码列表
    codes = ['601021','002027','002304']
    names = ['春秋航空','分众传媒','洋河股份']
    count = 0

    # 开始日期
    start_date = '20230801'
    # 结束日期
    dt_now = datetime.today()  # 取当前日期
    end_date = dt_now.strftime("%Y%m%d")  # 转换为20230822格式
    # end_date = '20230822'

    for code in codes:
        print(f'正在获取 {code} 从 {start_date} 到 {end_date} 的数据......')
        # 根据股票代码、开始日期、结束日期获取指定股票代码指定日期区间的k线数据
        df = get_k_history(code, start_date, end_date)
        # 保存k线数据到表格里面
        df.to_csv(f'20_Invest/stock/{count+1} - {code} - {names[count]}.csv', encoding='utf-8-sig', index=None)
        print(f'股票 {code} - {names[count]} 的 k线数据已保存到代码目录下的 {code}.csv 文件中')
        count = count+1
    print('全部获取完毕！')